/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */
/*  AES implementation in JavaScript                                        (c) Lucas Souza 2022  */
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */
 ...
 */
const cipher = (input: number[], w: number[][]) : number[] => {
  ...
  for (let i : number=0; i < 4*Nb; i++) output[i] = state[i%4][Math.floor(i/4)];
 ...
 * @private
 */
const subBytes = (s: number[][], Nb: number) : number[][] => {
  for (let r=0; r < 4; r++) {
 ...
 * @private
 */
const shiftRows = (s: number[][], Nb: number) : number[][] => {
  const t = new Array(4);
  for (let r=1; r<4; r++) {
 ...
 * @private
 */
const mixColumns = (s: number[][], Nb: number) : number[][] => {
  for (let c=0; c<4; c++) {
 ...
 * @private
 */
const addRoundKey = (state: number[][], w: number[][], rnd: number, Nb: number) => {
  for (var r=0; r<4; r++) {
 ...
 */
const encrypt = (plaintext: string, password: string, nBits: number) : string => {
  ...
  const nonce: number = (new Date()).getTime();  // timestamp: milliseconds since 1-Jan-1970
  const nonceMs: number = nonce%1000;
  const nonceSec: number = Math.floor(nonce/1000);
  const nonceRnd: number = Math.floor(Math.random()*0xffff);
  ...
  const ciphertext: string = ctrTxt + ciphertxt.join('');
 ...
 */
const decrypt = (ciphertext: string, password: string, nBits: number) => {
  const blockSize = 16;  // block size fixed at 16 bytes / 128 bits (Nb=4) for AES
  if (!(nBits==128 || nBits==192 || nBits==256)) return ''; // standard allows 128/192/256 bit keys
  ...
  const ciphertextArr = ct;

  // plaintext will get generated block-by-block into array of block-length strings
  const plaintxt = new Array(ciphertextArr.length);
  ...
  for (let b=0; b<nBlocks; b++) {
    ...
    const plaintxtByte = new Array(ciphertextArr[b].length);
    for (let i=0; i < ciphertextArr[b].length; i++) {
      // -- xor plaintxt with ciphered counter byte-by-byte --
      plaintxtByte[i] = cipherCntr[i] ^ ciphertextArr[b].charCodeAt(i);
