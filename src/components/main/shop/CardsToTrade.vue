...
<template>
  <div class="flex flex-col h-full card-list-names bg-gray-800 rounded-lg cursor-pointer p-4 gap-3">
    ...
    <ul class="flex flex-col h-full card-list-names cursor-pointer p-4 custom-scrollbar overflow-y-scroll">
      <li
      ...
      <template v-else>
        <li
          ...
          :class="{
            'bg-gray-600': chosenCard?.id === card.id && chosenCard.type === card.type,
